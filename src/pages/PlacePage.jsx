...
import { PLACE_OPTIONS } from '../data/config'
...
export default function PlacePage() {
  ...
  const { state, togglePlace } = useDate()
  const { advancing } = useAutoAdvanceOnSelection(state.places, '/when', 1000)
        ...
        <div className="relative z-10">
          ...
          >
            ...
            <p className="mt-2 font-body text-cream-dim text-xs sm:text-sm">
              I'll make the day feel special. ❤️
            </p>
          </motion.div>
          ...
          >
            {PLACE_OPTIONS.map((opt) => (
              <motion.div key={opt.id} variants={{ hidden: { opacity: 0, y: 12 }, show: { opacity: 1, y: 0 } }}>
                <SelectCard option={opt} selected={state.places.includes(opt.id)} onToggle={togglePlace} />
              </motion.div>
            ))}
          </motion.div>
          ...
          <div className="mt-7 flex flex-col items-center gap-3 min-h-[2rem]">
            {advancing && (
              ...
              >
                ...
                perfect pick, continuing...
              </motion.p>
            )}
            <BackLink onClick={() => navigate('/food')} />
          </div>
        </div>
      </Frame>
    </div>
